...
exports.create = async (req, res) => {
  try {
    const user = new User({
      ...
      selectedState: req.body.selectedState || undefined,
    });
    const savedUser = await user.save();
    return res.status(201).json({ status: true, message: messages.create.success, data: savedUser });
  } catch (error) {
    console.error("Error creating user:", error);
    return res.status(500).json({ status: false, message: error.message || messages.create.error });
  }
};
...
exports.getAll = async (req, res) => {
  try {
    ...
    let count = await User.countDocuments(query); // ✅ FIXED HERE
...
//Corewave make new api
exports.downloadDateRegistration = async (req, res) => {
  let userDetail = await userHelper.detail(req.headers["access-token"]);
  if (userDetail.status === 1 && userDetail.data.isAdmin === true) {
    ...
    try {
      ...
      if (!date) { return res.status(400).send({ status: false, message: "Please provide a date in YYYY-MM-DD format." }) }

      const userDate = new Date(date); // Convert query date to Date object
      const startDate = new Date(Date.UTC(userDate.getFullYear(), userDate.getMonth(), userDate.getDate(), 0, 0, 0, 0));
      const endDate = new Date(Date.UTC(userDate.getFullYear(), userDate.getMonth(), userDate.getDate(), 23, 59, 59, 999));


      let query = { isAdmin: false, isDeleted: false, createdAt: { $gte: startDate, $lte: endDate } };
      ...
      if (userData.length === 0) {
        return res.status(404).send({ status: false, message: "No users found for the given date." });
      }
      ...
      const filePath = `./uploads/temp/users_${date}.xlsx`;
      await workbook.xlsx.writeFile(filePath);
      res.send({ status: true, message: "Excel file generated successfully.", path: `${config.imageUrl}/temp/users_${date}.xlsx` });
    } catch (err) {
...
exports.check = (req, res) => {
  try {
    ...
    console.log("ehllo check")
    res.send({ status: true, message: "Check route hit" });
  } catch (err) {
...
//   try {
//     // Find user with the given email, excluding admin or deleted users
//     const user = await User.findOne({ email: req.body.email, isAdmin: false, isDeleted: false })
//     .populate("badges", "name description icon");
//     if (!user) {
//       return res.status(404).send({ status: false, message: messages.login.error });
//     }

//     // Check if password is correct
//     const isPasswordValid = bcrypt.compareSync(req.body.password, user.password);
//     if (!isPasswordValid) {
//       return res.status(401).send({ status: false, message: messages.login.error });
//     }

//     // Fetch user badges
//     const badges = await UserBadges.find({ user_id: user._id }, { __v: 0, auto: 0 })
//       .populate("badge_id", "name icon")
//       .sort({ _id: -1 })
//       .lean();

//     const badgeList = badges.map((d) => ({
//       badge_name: d.badge_id?.name || "",
//       badge_icon: d.badge_id?.icon ? config.imageUrl + d.badge_id.icon : "",
//     }));

//     // Generate access token
//     const token = await TokenModel.get(user._id, user.isAdmin);
//     console.log(token,"Token");
//     if (!token.status) {
//       return res.status(500).send({ status: false, message: "Error generating access token" });
//     }
...
exports.socialSignUp = async (req, res) => {
  ...
  if (source === "facebook") {
    ...
    checkUser = await User.findOne({ appleId: socialId, isDeleted: false }, { first_name: 1, last_name: 1, email: 1, profile_picture: 1, address: 1, pincode: 1, user_type: 1, points_balance: 1, state: 1, city: 1 }).lean();
  } else {
...
exports.feedStatistics = async (req, res) => {
  let userDetail = await userHelper.detail(req.headers["access-token"] || req.headers["authorization"]);
  const feedCount = await userPost.countDocuments({ isDeleted: false, user: userDetail.data.user_id });
  const commentCount = await Comment.countDocuments({ isDeleted: false, created_by: userDetail.data.user_id });
  const likeCount = await userLike.countDocuments({ user: userDetail.data.user_id });
  return res.send({
    status: true, data: {
...
exports.myEvents = async (req, res) => {
  ...
  try {
    let userDetail = await userHelper.detail(
      req.headers["access-token"] || req.headers["authorization"]
    );

    let event_type = req.body.event_type || null;
    const currentDate = new Date();
    let eventQuery = { $gte: ["$start_date", currentDate] };

    if (event_type === "past") {
      eventQuery = { $lt: ["$start_date", currentDate] };
    }
    ...
    let eventData = await Rsvp.aggregate([
      { $match: query },
      {
        $lookup: {
          from: "events",
      ...
      { $unwind: { path: "$eventList", preserveNullAndEmptyArrays: false } },
      {
        $project: {
          ...
          createdAt: 1,
          updatedAt: 1,
          title: "$eventList.title.en",
          last_date_to_enroll: "$eventList.last_date_to_enroll",
          event_type: "$eventList.event_type",
          photo: "$eventList.photo",
          platform: "$eventList.platform",
          location: "$eventList.location",
          start_date: "$eventList.start_date"
        }
      }
    ]);

    for (let i = 0; i < eventData.length; i++) {
      eventData[i].platform = eventData[i].platform || "";
      eventData[i].photo = eventData[i].photo
        ? config.imageUrl + eventData[i].photo
...
exports.myFeeds = async (req, res) => {
  ...
  limit = limit > config.limit ? config.limit : limit
  let offset = parseInt(req.query.offset ? req.query.offset : config.offset)
  let query = { user: new mongoose.Types.ObjectId(userDetail.data.user_id), isDeleted: false }
  try {
...
exports.appUsage = async (req, res) => {
  ...
  let query = {}
  let count = await AppUsage.countDocuments(query);
...
exports.getUserEngagement = async (req, res) => {
  ...
  let count = await UserEngagement.countDocuments(query)
...
exports.getUserBadges = async (req, res) => {
  ...
  let offset = parseInt(req.body.offset ? req.body.offset : config.offset)
  let query = {}
  let count = await UserBadges.countDocuments(query)
